...
章節學習地圖使用 Notion `code` block，語言設定為 `mermaid`。這是 Notion API 支援的 code block language，可讓 Notion 以 Mermaid 圖表方式呈現；送出前會移除 AI 可能產生的 Markdown code fence，避免 Notion 把 ```mermaid 當成普通文字。

Mermaid 在送出前會先做 label 安全化。HTML/CSS 教材常會把 `<div>`、`#id`、`a|b`、`color: red` 放進學習地圖，這些符號若直接出現在 Mermaid 節點文字，可能被解析器當成語法。`mermaid_validator.py` 會把節點標籤內的高風險符號轉成全形顯示字元，保留教學意思，同時降低 Notion 無法渲染的機率。

PDF 圖片使用 Notion File Upload API。系統先建立 file upload、送出圖片 bytes，再用 `type: image`、`image.type: file_upload` 和 upload ID 附加到章節子頁。若快取裡只有頁碼與圖片解讀，且背景工作仍有原始 PDF，系統會依頁碼重新渲染圖片，不重跑 AI。
...
AI 生成現在可由 `AI_PROVIDER` 切換：

```text
openai -> src/services/openai_service.py
gemini -> src/services/gemini_service.py
```

`analysis_service.py`、`chapter_service.py` 與 `pdf_visual_service.py` 都會讀取同一個 `AI_PROVIDER`。因此選擇 Gemini 時，文件摘要、合併摘要、章節詳細筆記與 PDF 圖片視覺分析都會改用 Gemini。Notion API、SQLite 與快取流程不需要改變。

Gemini 使用 Google 官方 `google-genai` SDK。文字生成使用 `client.models.generate_content()`；PDF 視覺分析會先把 PDF 頁面渲染成圖片，再把 prompt 與圖片一起送給 Gemini 多模態模型。
