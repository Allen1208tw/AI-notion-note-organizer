...
def build_chapters_from_headings(
    ...
    return disambiguate_duplicate_chapter_titles(
        chapters
    )
...
def disambiguate_duplicate_chapter_titles(
    ...
    有些教材會連續多章使用相同大標題，例如「CSS樣式屬性」，
    真正差異藏在章內子主題。這裡只處理同名主章節，不影響標題
    本來就唯一的教材。
    """
    ...
    for chapter in chapters:
        ...
        title_counts[title_key] = (
            title_counts.get(title_key, 0) + 1
        )
    ...
    for chapter in chapters:
        ...
        suffix = build_duplicate_title_suffix(
            title,
            chapter,
        )
        ...
        chapter["title"] = clean_heading_title(
            f"{title}｜{suffix}"
        )
...
def build_duplicate_title_suffix(
    base_title: str,
    chapter: dict,
) -> str:
    """從章內子章節建立同名主章節的辨識用短標。"""
    ...
    for subsection in chapter.get("subsections") or []:
        ...
        topic_key = normalize_for_compare(topic)

        if not topic_key or topic_key == base_key:
            continue
